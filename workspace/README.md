...
2. Merge the `\ISO EDITS` contents ('files' and 'sys') with the extracted game. If making a non-widescreen build, merge `\workspace\Widescreen-ShadowTH-overrides\4x3\` afterwards.
...
4. Open the `\workspace\fileToDelete-every-release.txt` and delete these directories and files from your extracted game, as specified.
5. Download MCM (Melee Code Manager) if you haven't: https://github.com/DRGN-DRC/Melee-Code-Manager/releases
6. You will need to overwrite configurations to make it work with Shadow. After extracting v4.4.1 or newer:
...
7. Launch MCM. Select your extracted DOL. Specify version 1.06 if asked. It should show all of the codes across the four files. Apply your desired patches. In the case of U&R, builds are to be made as follows:
* **Widescreen**: All but SX patches enabled.
* **Standard**: All but SX patches and the 'Advanced Full Widescreen' patch enabled.
* **SX**: All enabled, no exceptions.
8. Build the ISO with Dolphin's -> Right Click Game -> 'Convert'. 
